...
#' @param depth1 gives the number of interacting covariates. If set to 1, then 
...
#' @param depth2 Controls the number of interacting covariates in the AE outcome. Defaults to 1. 
#' @param cut.type Indicates the type of subgroup structure (linear or single)
...
#' @param beta21 controls the strength of the control effect for AE outcome. Defaults to 1. 
...
#' @param cut1,cut2,cut3 controls where the cutpoints are to define subgroups. 
...
mvdatAlt <- function(n = 500, ## The sample size
                     K = 50,  ## The thinness of the uniform generator (1/K)
                     K.E = 10, ## The thinness of the uniform generator for excess covariates
                     design = c("emr", "rct"), ## Design of the study
                     cut.type = c("linear", "single"), ## defines the subgroup
                     beta11 = 1, ## Effect for T == 1 and in subgroup
                     beta12 = 1, ## Effect for T == 0 and not in subgroup
                     beta21 = 1, ## Effect for AE subgroup
                     ...
                     depth2 = 1,  ## Controls the number of interating covariates in AE outcome subgroup
                     ...
                     sigma = 0.5) ## Standard deviation for the errors in log survival time
{
  ...
  if(cut.type == "linear"){
    if(depth1 == 1){
      design.mat1 <- as.matrix(cbind(1, X, E, (X[,1] - 0.5)*trt))
      betas1 <- c(4, 0, 2, rep(0, 2 + n.noise.cov), beta11)
    } else{
      stopifnot(depth1 == 2)
      design.mat1 <- as.matrix(cbind(1, X, E, (X[,1] - 0.5)*(X[,3] - 0.5)*trt))
      betas1 <- c(4, 0, 2, rep(0, 2 + n.noise.cov), beta11)
    }
  } else if(cut.type == "single"){
    design.mat1 <- as.matrix(cbind(1, X, E, trt*(X[,1] < cut1), (1-trt)*(1 - (X[,1] < cut1))))
    betas1 <- c(4, 0, 2, rep(0, 2 + n.noise.cov), beta11, beta12)
  }
  
  status <- rbinom(n, 1, 1-rateC)
  time <- as.vector(offset1*(-log(runif(n, 0, 1)) * exp(-betas1 %*% t(design.mat1)) / lambda)^(1 / rho))
  
  # Obtain KM estimate of survival
  surv <- survfit(Surv(time, status)~1)
  ...
  if(cut.type == "linear"){
    if(depth2 == 1){
      design.mat2 <- as.matrix(cbind(1, log(time), X, E, (X[,2] - 0.5)*(X[,1] - 0.5 < 0)*trt))
      betas2 <- c(0.25, 1, 0.25, rep(0, 3 + n.noise.cov), beta21)
    } else{
      stopifnot(depth2 == 2)
      design.mat2 <- as.matrix(cbind(1, log(time), X, E, 
                                     (X[,2] - 0.5)*(X[,4] - 0.5)*(X[,1] - 0.5 < 0)*(X[,3] - 0.5 < 0)*trt))
      betas2 <- c(0.25, 1, 0.25, rep(0, 3 + n.noise.cov), beta21)
    }
  } else if(cut.type == "single"){
    design.mat2 <- as.matrix(cbind(1, log(time), X, E))
    betas2 <- c(0.25, 1, 0.25, rep(0, 3 + n.noise.cov))
  }
