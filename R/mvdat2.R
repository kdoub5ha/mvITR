...
mvdat2 <- function(n = 500, ## The sample size
                   ...
                   sigma = 0.5) ## Standard deviation for the errors in log survival time
{
  ...
  betas1 <- c(2, 0, 0.25, -0.25, rep(0, 2 + n.noise.cov), beta1, beta1)
  status <- rbinom(n, 1, 1-rateC)
  time <- as.numeric((-log(runif(n, 0, 1)) / (exp(betas1 %*% t(design.mat1))*lambda))^(1 / rho))
  ...
  surv.c <- survfit(coxph(Surv(time, 1-status) ~ ., data = df.tmp), data = df.tmp)
  ...
  design.mat2 <- as.matrix(cbind(1, log(time), X, E, 
                                 (1-subgroup1)*subgroup21*trt, 
                                 (1-subgroup1)*(1-subgroup21)*(1-trt), 
                                 subgroup1*subgroup22*trt, 
                                 subgroup1*(1-subgroup22)*(1-trt)))
