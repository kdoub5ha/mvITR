...
{   
  ...
  if(name == "0"){
    ...
    dat.comb <- as.data.frame(
      rbind(dat.rest[, c('y', 'trt', 'prtx', 'status', 'KM.cens')], 
            dat[,c('y', 'trt', 'prtx', 'status', 'KM.cens')])
    )
  }
  ...
  if (depth <= max.depth && n >= min.ndsz) {
    ...
    outSplit <- lapply(splitVars, function(i){
      ...
      if(length(temp) > 1) {
        ...
        # the following is a hacky way to deal with scrolling 
        #  through categorical and continuous predictors
        if(is.ctg){
        ...
        if(name == "0"){
          ...
          datMatrix <- list(y = dat.comb$y, 
                            x = x.tmp, 
                            prtx = dat.comb$prtx,
                            status = as.numeric(dat.comb$status), 
                            kmCens = dat.comb$KM.cens,
                            trt = dat.comb$trt, 
                            trtNew = rep(0, length(x)),
                            inNode = rep(1, length(x)))
        } else{
          # browser()
          x.tmp <- ifelse(rep(!is.ctg, length(c(dat.rest[,i], x))), 
                          c(dat.rest[,i], x), as.numeric(factor(c(dat.rest[,i], x))))
          datMatrix <- list(y = dat.comb$y, 
                            x = x.tmp, 
                            prtx = dat.comb$prtx, 
                            status = as.numeric(dat.comb$status), 
        ...
        if(outcome == "time"){
          tmp <- splitSurvival(zcut, zcutCat, datMatrix, splitParams)
        } else{
          stopifnot(outcome == "ae")
          tmp <- splitContinuous(zcut, zcutCat, datMatrix, splitParams)
        }
        return(tmp)
      } else{
